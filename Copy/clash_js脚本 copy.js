function main(config) {
  ...
  // 【核心】Claude强制走链式代理（优先级最高）
  if (!config["rules"]) config["rules"] = [];
  config["rules"].unshift("DOMAIN-SUFFIX,anthropic.com,🗽美国AI(链式)");
  config["rules"].unshift("DOMAIN-SUFFIX,claude.ai,🗽美国AI(链式)");
  ...
  config["proxy-groups"] = [
    {
      ...
      "name": "✈️开启代理",
      "type": "select",
      "proxies": ["DIRECT", "🇭🇰香港测速⚡", "🇯🇵日本测速⚡", "🇰🇷韩国测速⚡", "🇺🇸美国测速⚡", "🇸🇬新加坡测速⚡", "👋避免香港IP检测", "💈负载均衡💈"],
    ...
    // ====================== 链式代理核心组（100%生效） ======================
    {
      ...
      "hidden": false,
      "include-all": false,
      // 关键：为每个美国节点设置dialer-proxy=香港中转
      "override": {
        "dialer-proxy": "🇭🇰香港测速⚡" // 先香港→再美国，实现链式
      },
      "proxies": ["🇺🇸美国节点"],
    ...
    },
    {
      ...
      "type": "select",
      "include-all": true,
      "filter": "(?i)^(?!.*(?:HK|hk|🇭🇰|hongkong|港|JP|jp|🇯🇵|Japan|日本|KR|kr|🇰🇷|korea|韩国|US|us|🇺🇸|America|美国|SG|sg|🇸🇬|singapore|新加坡|🏷|t\\.me)).*",
